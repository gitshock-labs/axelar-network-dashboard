...
export const transactions = async (params, denoms) => {
  ...
  if (response?.hits?.hits) {
    response.hits.hits = response.hits.hits.map(hit => {
      return {
        ...hit._source,
        ...hit.fields,
        status: tx_manager.status(hit.fields || hit._source),
        type: tx_manager.type(hit.fields || hit._source),
        fee: tx_manager.fee(hit.fields || hit._source, denoms),
        symbol: tx_manager.symbol(hit.fields || hit._source, denoms),
        gas_used: tx_manager.gas_used(hit._source),
        gas_limit: tx_manager.gas_limit(hit._source),
        memo: tx_manager.memo(hit._source),
        activities: tx_manager.activities(hit._source),
      }
    })
  ...
  if (_response?.aggregations?.types?.buckets) {
    response = { data: _response.aggregations.types.buckets.map(record => record?.key).filter(t => t) }
  }
