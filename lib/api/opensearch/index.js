...
export const transfers = async params => {
  ...
  if (response?.aggregations?.transfers?.buckets) {
    response = {
      data: response.aggregations.transfers.buckets.map(record => { return { contract_name: record.key, tx: record.doc_count, amount: record.amounts?.value, since: record.since ? record.since.value : undefined } }),
      total: response.hits && response.hits.total && response.hits.total.value,
    }
  }
