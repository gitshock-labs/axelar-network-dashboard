...
export const heartbeats = async params => {
  ...
  if (response?.aggregations?.heartbeats?.buckets) {
    response = {
      data: Object.fromEntries(response.aggregations.heartbeats.buckets.map(record => [base64ToBech32(record.key, process.env.NEXT_PUBLIC_PREFIX_CONSENSUS), record.doc_count])),
      total: response.hits?.total?.value,
    }
  }
