...
export const heartbeats = async params => {
  ...
  if (response?.aggregations?.heartbeats?.buckets) {
    response = {
      data: Object.fromEntries(response.aggregations.heartbeats.buckets.map(record => [record.key, record.heightgroup?.buckets && record.heightgroup?.buckets.length <= 1000 ? record.heightgroup.buckets.length : record.doc_count])),
      total: response.hits?.total?.value,
    }
  }
