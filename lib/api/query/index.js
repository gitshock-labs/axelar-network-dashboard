...
export const denoms = async params => {
  params = { ...params, collection: `denoms_${process.env.NEXT_PUBLIC_NETWORK}` }
...
//     if (response?.hits?.hits) {
//       data = _.orderBy(_.concat(data || [], response.hits.hits.map(uptime => { return { ...uptime?.fields, height: uptime?.fields?.height?.[0], timestamp: uptime?.fields?.timestamp?.[0], validators: uptime?.fields?.validators?.map(v => base64ToBech32(v, process.env.NEXT_PUBLIC_PREFIX_CONSENSUS)) } }).map(uptime => {
//         return {
