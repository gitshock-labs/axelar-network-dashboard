    ...
    query: {
      bool: {
        must: [
          ...
          { range: { height: { gt: beginBlock, lte: latestBlock } } },
        ],
      },
    },
    size: Number(process.env.NEXT_PUBLIC_NUM_HEARTBEAT_BLOCKS) / Number(process.env.NEXT_PUBLIC_NUM_BLOCKS_PER_HEARTBEAT),
  })
...
export const getIneligibilities = async params => {
  const response = await prometheus({ ...params, query: '{__name__=~"axelar_tss_heartbeat"}[1d]' })
