...
import { denomAmount, denomSymbol } from '../../object/denom'
...
export const allValidators = async (params, validatorsData, status, addresses, latestBlock, denoms, isSupportedChains) => {
  ...
  data = data.filter(validator_data => !['genesis'].includes(validator_data?.description?.moniker))
...
export const allProposals = async (params, denoms_data) => {
  let pageKey = true
    ...
    data = _.orderBy(_.uniqBy(_.concat(data, response.proposals?.map(_proposal => {
      return {
        ...
        total_deposit: _proposal?.total_deposit?.map(deposit => {
          return {
            ...deposit,
            amount: denomAmount(deposit?.amount, deposit.denom, denoms_data),
            symbol: denomSymbol(deposit?.denom, denoms_data),
          }
        }),
...
export const proposal = async (id, params, denoms_data) => {
