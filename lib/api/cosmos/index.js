...
export const allValidators = async (params, validatorsData, status, addresses, latestBlock, denoms, isSupportedChains) => {
  ...
  if (status || addresses) {
    ...
    if (data) {
      const should_have_broadcaster_data = data.filter(validator_data => !validator_data.broadcaster_address && !validator_data.tss_illegibility_info?.stale_tss_heartbeat)
