...
export const validatorSelfDelegation = async (validator_data, validatorsData, status) => {
  if (validator_data) {
    if (validator_data.delegator_address && typeof validator_data.self_delegation !== 'number') {
      ...
      else if (status && (typeof status === 'boolean' || (['active'].includes(status) ? ['BOND_STATUS_BONDED'].includes(validator_data.status) : ['deregistering'].includes(status) ? true : !(['BOND_STATUS_BONDED'].includes(validator_data.status))))) {
