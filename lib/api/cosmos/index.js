...
export const allValidators = async (params, validatorsData, status, addresses, latestBlock, denoms) => {
  ...
  if (latestBlock && (['active', 'deregistering'].includes(status) || addresses)) {
  ...
  if (status || addresses) {
    ...
    if (response) {
      data = data.map(validator_data => {
        const tx = response.find(_tx => _tx && !_tx.code && _tx.activities?.findIndex(activity => activity?.sender === validator_data?.operator_address) > -1)
