...
export const transactionsByEvents = async (events, data, validator, isUnlimit) => {
  ...
  while ((pageKey || total) && txsData.length < total && (!isUnlimit || txsData.length < max_size)) {
    const response = await transactions({ events, 'pagination.key': pageKey && typeof pageKey === 'string' ? pageKey : undefined, 'pagination.offset': pageKey && typeof pageKey === 'string' ? undefined : txsData.length }, validator)
