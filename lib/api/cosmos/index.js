...
import { base64ToHex, base64ToBech32, delegatorAddress, pubKeyToBech32, pubKeyToBech32secp256k1 } from '../../object/key'
  ...
  if (response && response.tx_responses) {
    response.tx_responses = response.tx_responses.map(record => {
      ...
      return {
        ...
        // type/*signer_address*/: record.tx && record.tx.auth_info && record.tx.auth_info.signer_infos && _.head(record.tx.auth_info.signer_infos) && _.head(record.tx.auth_info.signer_infos).public_key && pubKeyToBech32secp256k1(_.head(record.tx.auth_info.signer_infos).public_key.key, process.env.NEXT_PUBLIC_PREFIX_ACCOUNT),
    ...
    for (let i = 0; i < validators_data.length; i++) {
      ...
      if (validator_data) {
        ...
        if (validator_data.delegator_address && typeof validator_data.self_delegation !== 'number') {
          ...
          else if ((addresses && addresses.includes(validator_data.operator_address)) || (status && (['active'].includes(status) ? ['BOND_STATUS_BONDED'].includes(validator_data.status) : !(['BOND_STATUS_BONDED'].includes(validator_data.status))))) {
            const responseSelfDelegation = await stakingDelegationsAddress(validator_data.operator_address, validator_data.delegator_address)
...
export const allValidators = async (params, validatorsData, status, addresses, latestBlock) => {
  ...
  if (status || addresses) {
    const response = await axelard({ cmd: 'axelard q tss deactivated-operators' })
