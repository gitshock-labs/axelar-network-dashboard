...
export const allValidatorsStatus = async validators_data => {
  ...
  if (convertToJson(response?.data?.stdout)) {
    const deregistering_addresses = convertToJson(response.data.stdout)?.operator_addresses || []
  ...
  response = await axelard({ cmd: 'axelard q snapshot validators -oj', cache: true, cache_timeout: 3 })

  if (convertToJson(response?.data?.stdout)) {
    const illegible_addresses = convertToJson(response.data.stdout)?.validators?.filter(v => Object.values(v?.tss_illegibility_info || {}).findIndex(value => value) > -1)
  ...
  if (data) {
    ...
    if (should_have_broadcaster_data.length > 1) {
      ...
      broadcasters_data = response?.data?.map(b => {
        const broadcaster_address = convertToJson(b?.data?.stdout)?.address
    ...
    for (let i = 0; i < should_have_broadcaster_data.length; i++) {
      ...
      if (!v.broadcaster_address) {
        ...
        if (!v.broadcaster_address) {
          response = await axelard({ cmd: `axelard q snapshot proxy ${v.operator_address}`, cache: true })

          if (convertToJson(response?.data?.stdout)) {
            v.broadcaster_address = convertToJson(response.data.stdout).address
          }
        }
...
export const chainMaintainer = async (id, chains) => {
  ...
  const _chains = chains?.filter(c => c?.id === id).map(c => c?.id) || []

  for (let i = 0; i < _chains.length; i++) {
    const id = _chains[i]

    const response = await axelard({ cmd: `axelard q nexus chain-maintainers ${chain_manager.maintainer_id(id, chains)} -oj`, cache: true, cache_timeout: 1 })

    if (convertToJson(response?.data?.stdout)?.maintainers) {
      data[id] = Object.values(convertToJson(response.data.stdout).maintainers)
    }
  }
