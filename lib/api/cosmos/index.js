...
import { chains, chainMaintainerId } from '../../object/chain'
...
export const validators = async (params, validators_data, status, addresses) => {
        ...
        if (_validator.description) {
          if (_validator.description.identity) {
            if (validators_data?.findIndex(v => v?.description?.image && v.operator_address === _validator.operator_address) > -1) {
              ...
              _validator.description.image = _validator.description.image || randImage(i)
            }
          }
          else {
            _validator.description.image = _validator.description.image || randImage(i)
...
export const allValidators = async (params, validators_data, status, addresses, latest_block, denoms) => {
  ...
  while (pageKey) {
    const response = await validators({ ...params, 'pagination.key': pageKey && typeof pageKey === 'string' ? pageKey : undefined }, validators_data, status, addresses)
  ...
  while (pageKey) {
    ...
    data = _.orderBy(_.uniqBy(_.concat(data, response.proposals?.map(_proposal => {
      return {
        ...
        content: {
          ..._proposal?.content,
          plan: { ..._proposal?.content?.plan, height: Number(_proposal?.content?.plan?.height) },
        },
        ...
        final_tally_result: Object.fromEntries(Object.entries(_proposal?.final_tally_result || {}).map(([key, value]) => [key, Number(value)]))
      }
  ...
  if (response?.tx_responses) {
    response.tx_responses = response.tx_responses.map(record => {
      ...
      return {
        ...
        vote: validator_address && activities ?
          activities.findIndex(a => a?.participants && JSON.parse(a.participants).includes(validator_address)) > -1 ?
            activities.findIndex(a => a?.participants && a.participantShareCounts && JSON.parse(a.participantShareCounts)[JSON.parse(a.participants).indexOf(validator_address)] !== '0') > -1 ?
              'approved'
              :
              'denied'
            :
...
export const transactionsByEvents = async (events, data, validator_address, isUnlimit, denoms) => {
  const page_size = 100, max_size = 500
...
export const transactionsByEventsPaging = async (events, data, offset, denoms) => {
  const page_size = 100, max_size = 100
  let total = 100, txs = []
...
export const validatorStatusData = async validatorsData => {
  let response = await axelard({ cmd: 'axelard q tss deactivated-operators', cache: true, cache_timeout: 1 })
  ...
  if (response?.data?.stdout) {
    ...
    data = _.orderBy(data.map(validator_data => { return { ...validator_data, deregistering: deregisteringAddresses.includes(validator_data.operator_address) || (['genesis'].includes(validator_data.description.moniker) && !['BOND_STATUS_BONDED'].includes(validator_data.status)) } }), ['deregistering', 'description.moniker'], ['asc', 'asc'])
  }

  response = await axelard({ cmd: 'axelard q snapshot validators -oj', cache: true, cache_timeout: 1 })
...
export const broadcastersData = async (validatorsData, addresses, denoms) => {
  addresses = addresses && !Array.isArray(addresses) ? [addresses] : addresses

  let data = validatorsData || []

  let response = await transactionsByEvents(`message.action='RegisterProxy'`, null, null, true, denoms)
  ...
  if (data) {
    const should_have_broadcaster_data = data.filter(validator_data => !validator_data.broadcaster_address/* && !validator_data.tss_illegibility_info?.stale_tss_heartbeat*/ && (!addresses || addresses.includes(validator_data.operator_address)))
    ...
    if (should_have_broadcaster_data.length > 1) {
      ...
      broadcasters_data = response?.data?.map(broadcaster_data => {
        let broadcaster_address

        try {
          broadcaster_address = JSON.parse(broadcaster_data?.data?.stdout).address
        } catch (error) {}

        return {
          operator_address: _.last(broadcaster_data?.id?.split(' ')),
          broadcaster_address,
        }
      })
    }
    ...
    for (let i = 0; i < should_have_broadcaster_data.length; i++) {
      const validator_data = should_have_broadcaster_data[i]

      if (!validator_data.broadcaster_address) {
        validator_data.broadcaster_address = broadcasters_data?.find(broadcaster_data => broadcaster_data?.operator_address === validator_data.operator_address)?.broadcaster_address

        if (!validator_data.broadcaster_address) {
          response = await axelard({ cmd: `axelard q snapshot proxy ${validator_data.operator_address}`, cache: true })

          if (response?.data?.stdout) {
            try {
              validator_data.broadcaster_address = JSON.parse(response.data.stdout).address
            } catch (error) {}
          }
        }
      }
...
export const chainMaintainer = async id => {
  const data = {}

  const _chains = chains.filter(_chain => !_chain.hidden && !_chain.is_cosmos && (!id || _chain?.id === id)).map(_chain => _chain?.id)

  for (let i = 0; i < _chains.length; i++) {
    const chain = _chains[i]

    const response = await axelard({ cmd: `axelard q nexus chain-maintainers ${chainMaintainerId(chain)} -oj`, cache: true, cache_timeout: 1 })

    if (convertToJson(response?.data?.stdout)?.maintainers) {
      data[chain] = Object.values(convertToJson(response.data.stdout).maintainers)
    }
  }
