...
export const getTxActivities = (data, denoms) => {
  const activities = data?.logs?.map(log => log?.events && _.assign.apply(_, (log.events.map(event => {
    const event_obj = {
      type: event.type,
      log: log.log, ...((event.attributes && _.assign.apply(_, event.attributes.filter(attribute => event.type !== 'message'/*!(event.type !== 'message' && attribute.key === 'action')*/).map(attribute => {
        const attr_obj = {
