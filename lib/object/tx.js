...
export const getTxActivities = data => {
  const activities = data && data.logs && data.logs.map(log => log && log.events && _.assign.apply(_, (log.events.map(event => { return { type: event.type, log: log.log, ...((event.attributes && _.assign.apply(_, event.attributes.filter(attribute => !(event.type !== 'message' && attribute.key === 'action')).map(attribute => { return { [`${attribute.key}`]: attribute.key === 'amount' && typeof attribute.value === 'string' ? Number(attribute.value.substring(0, attribute.value.split('').findIndex(c => isNaN(c)) > -1 ? attribute.value.split('').findIndex(c => isNaN(c)) : undefined)) / Math.pow(10, attribute.value.startsWith('u') ? 6 : 0) : attribute.value, symbol: attribute.key === 'amount' && typeof attribute.value === 'string' ? attribute.value.substring(attribute.value.split('').findIndex(c => isNaN(c))).substring(attribute.value.startsWith('u') ? 1 : 0) : undefined } }))) || {}) } }))))
