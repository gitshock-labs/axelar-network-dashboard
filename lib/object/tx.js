...
export const getTxType = data => {
  let type = _.head(data?.logs?.flatMap(log => log?.events?.filter(event => event.type === 'message').map(event => event.type === 'message' ? event.attributes && _.head(event.attributes.filter(attribute => attribute.key === 'action').map(attribute => _.last(attribute.value?.split('.') || []))) : event.type)))
