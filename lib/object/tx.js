...
export const getTxActivities = data => {
  const activities = data && data.logs && data.logs.map(log => log && log.events && _.assign.apply(_, (log.events.map(event => { return { type: event.type, log: log.log, ...((event.attributes && _.assign.apply(_, event.attributes.filter(attribute => !(event.type !== 'message' && attribute.key === 'action')).map(attribute => { return { [`${attribute.key}`]: attribute.key === 'amount' && typeof attribute.value === 'string' ? denomAmount(attribute.value.substring(0, attribute.value.split('').findIndex(c => isNaN(c)) > -1 ? attribute.value.split('').findIndex(c => isNaN(c)) : undefined), attribute.value.substring(attribute.value.split('').findIndex(c => isNaN(c)))) : attribute.value, symbol: attribute.key === 'amount' && typeof attribute.value === 'string' ? denomName(attribute.value.substring(attribute.value.split('').findIndex(c => isNaN(c)))) : undefined } }))) || {}) } }))))
