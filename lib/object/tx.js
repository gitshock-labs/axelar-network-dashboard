...
export const getTxType = data => {
  ...
  if (data && !data.code &&
    Number(data?.height) % Number(process.env.NEXT_PUBLIC_NUM_BLOCKS_PER_HEARTBEAT) === 1 &&
    data?.tx?.body?.messages?.findIndex(message => message?.inner_message?.['@type']?.includes('HeartBeatRequest')) > -1
  ) {
    type = 'Heartbeat'
  }
