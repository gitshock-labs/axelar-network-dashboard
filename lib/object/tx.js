...
import { denoms, feeDenom, denomName, denomAmount } from './denom'
    ...
    const event_obj = {
      ...
      log: log.log, ...((event.attributes && _.assign.apply(_, event.attributes.filter(attribute => !(event.type !== 'message' && attribute.key === 'action')).map(attribute => {
        ...
        if (attribute.key === 'amount' && typeof attribute.value === 'string') {
          attr_obj.symbol = denomName(attribute.value.split('').findIndex(c => isNaN(c)) > -1 ? attribute.value.substring(attribute.value.split('').findIndex(c => isNaN(c))) : denoms?.[0]?.denom)
        }
