...
export const getTxType = data => {
  let type = _.head(data?.logs?.flatMap(log => log?.events?.filter(event => event.type === 'message'/*event.type !== 'message' || log.events.length < 2*/).map(event => event.type === 'message' ? event.attributes && _.head(event.attributes.filter(attribute => attribute.key === 'action').map(attribute => _.last(attribute.value?.split('.') || []))) : event.type)))

  if (data) {
    if (!data.code) {
      if (data?.tx?.body?.messages?.findIndex(message => message?.inner_message?.['@type']) > -1) {
        type = _.last(data.tx.body.messages.find(message => message?.inner_message?.['@type']).inner_message['@type'].split('.'))
      }
    }
    else {
      if (data?.tx?.body?.messages?.findIndex(message => message?.['@type']) > -1) {
        type = _.last(data.tx.body.messages.find(message => message?.['@type'])['@type'].split('.'))
      }
    }
  }

  return type?.endsWith('Request') ? type.replace('Request', '') : type
}
