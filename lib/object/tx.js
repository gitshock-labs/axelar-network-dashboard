...
import { denoms, feeDenom, denomSymbol, denomAmount } from './denom'

export const getTxStatus = data => data && !data.code ? 'success' : 'failed'

export const getTxType = data => _.head(data?.logs?.flatMap(log => log?.events?.filter(event => event.type !== 'message' || log.events.length < 2).map(event => event.type === 'message' ? event.attributes && _.head(event.attributes.filter(attribute => attribute.key === 'action').map(attribute => attribute.value)) : event.type)))

export const getTxFee = data => data?.tx?.auth_info?.fee?.amount && denomAmount(_.sumBy(data.tx.auth_info.fee.amount, 'amount'), feeDenom)

export const getTxSymbol = data => data?.tx?.auth_info?.fee?.amount && _.head(data.tx.auth_info.fee.amount.map(amount => denomSymbol(amount?.denom)).filter(denom => denom))
    ...
    const event_obj = {
      ...
      log: log.log, ...((event.attributes && _.assign.apply(_, event.attributes.filter(attribute => !(event.type !== 'message' && attribute.key === 'action')).map(attribute => {
        ...
        if (attribute.key === 'amount' && typeof attribute.value === 'string') {
          attr_obj.symbol = denomSymbol(attribute.value.split('').findIndex(c => isNaN(c)) > -1 ? attribute.value.substring(attribute.value.split('').findIndex(c => isNaN(c))) : denoms?.[0]?.denom)
        }
