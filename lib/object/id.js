export const type = id => {
  ...
  const accountOsmosisRegEx = new RegExp(`${process.env.NEXT_PUBLIC_PREFIX_ACCOUNT.replace('axelar', 'osmo')}.*$`, 'igm')

  return !id ? null : !isNaN(id) ? 'block' : id.match(validatorRegEx) ? 'validator' : id.match(accountRegEx) || id.match(accountCosmosRegEx) || id.match(accountTerraRegEx) || id.match(accountOsmosisRegEx) ? 'account' : 'tx'
}