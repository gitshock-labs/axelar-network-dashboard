      ...
      if (chains_data && cosmos_chains_data && denoms_data) {
        ...
        _data = Object.entries(_.groupBy(_data, 'id')).map(([key, value]) => {
          return {
            ...
            avg_amount: _.mean(value.map(v => v.tx * v.avg_amount)) / _.sumBy(value, 'tx'),
            avg_value: _.mean(value.map(v => v.tx * v.avg_value)) / _.sumBy(value, 'tx'),
          ...
          return {
            ...
            times: Object.entries(_.groupBy(value?.flatMap(v => v.times || []) || [], 'time')).map(([_key, _value]) => {
              return {
                ...
                avg_amount: _.mean(_value.map(v => v.tx * v.avg_amount)) / _.sumBy(_value, 'tx'),
                avg_value: _.mean(_value.map(v => v.tx * v.avg_value)) / _.sumBy(_value, 'tx'),
          ...
          return {
            ...
            times: Object.entries(_.groupBy(value?.flatMap(v => v.times || []) || [], 'time')).map(([_key, _value]) => {
              return {
                ...
                amount: _.sumBy(_value, 'amount'),
                value: _.sumBy(_value, 'value'),
                avg_amount: _.mean(_value.map(v => v.tx * v.avg_amount)) / _.sumBy(value, 'tx'),
                avg_value: _.mean(_value.map(v => v.tx * v.avg_value)) / _.sumBy(value, 'tx'),
                max_amount: _.maxBy(_value, 'max_amount')?.max_amount,
                max_value: _.maxBy(_value, 'max_value')?.max_value,
              }
            }),
