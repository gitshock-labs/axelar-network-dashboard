...
export default function TimelyHighestTransfer({ highestTransferData, setTimeFocus }) {
  ...
  useEffect(() => {
    if (highestTransferData) {
      const _data = highestTransferData.times?.map((timely, i) => {
        return {
          ...timely,
      ...
      {loaded ?
        <ResponsiveContainer>
          ...
          >
            ...
            <Bar dataKey="amount" minPointSize={5}>
              {data.map((entry, i) => (<Cell key={i} fillOpacity={1} fill="url(#gradient-high)" />))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
        :
