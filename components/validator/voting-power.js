    ...
    >
        ...
        <div className="flex items-center sm:justify-center mt-5 mb-6">
          <div className="w-60 h-32 bg-gray-900 dark:bg-black rounded-lg flex items-center justify-center">
            <div className="flex flex-col text-center space-y-1">
              <span className="text-white text-2xl font-semibold">{numberFormat(data.voting_power || Math.floor(denomer.amount(data.tokens, denoms_data?.[0]?.denom, denoms_data)), '0,0')}</span>
              {env_data?.staking_pool?.bonded_tokens && (
                <span className="text-gray-200 dark:text-gray-200 text-sm">(~ {numberFormat(data.voting_power || Math.floor(denomer.amount(data.tokens, denoms_data?.[0]?.denom, denoms_data)) * 100 / Math.floor(env_data.staking_pool.bonded_tokens), '0,0.00')}%)</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div className={`grid grid-flow-row grid-cols-1 sm:grid-cols-2 text-base sm:text-sm lg:text-base gap-4 ${data ? '' : 'my-1.5'}`}>
          ...
          {data ?
            <span className="flex items-center text-gray-500 dark:text-gray-400 space-x-1.5">
              ...
              <span className="text-gray-500 space-x-1">
                <span>(~</span>
                <span>{numberFormat(Math.floor(denomer.amount(data.self_delegation, denoms_data?.[0]?.denom, denoms_data)), '0,0')}</span>
        ...
        <div className={`flex flex-col space-y-${data ? 1 : 2}`}>
          ...
          {data ?
            <span className="text-gray-500 dark:text-gray-400">
              {numberFormat(denomer.amount(data.delegator_shares, denoms_data?.[0]?.denom, denoms_data), '0,0')}
            </span>
            :
        ...
        <div className={`flex flex-col space-y-${data ? 1 : 2}`}>
          ...
          {data ?
            <span className="text-gray-500 dark:text-gray-400">
              {numberFormat(Math.floor(denomer.amount(data.tokens, denoms_data?.[0]?.denom, denoms_data)), '0,0')}
            </span>
            :
            <div className="skeleton w-20 h-6" />
          }
